...
check(
  "员工含 restMinutes",
  d.employees.every((e) => typeof e.restMinutes === "number"),
);
check(
  "员工含 insuranceBase",
  d.employees.every((e) => "insuranceBase" in e),
);
const firstRec = Object.values(d.attendance[0].rec)[0];
check(
  "考勤已升级为分时段对象 {am,pm,ot}",
  !!(
    firstRec &&
...
check(
  "attendance summary 键齐全",
  d.attendance.every((a) => a.summary && "出勤" in a.summary && "加班" in a.summary),
);
check(
  "payroll 含缴纳明细(comp/pers)",
  d.payroll.every((p) => p.comp && p.pers),
);
