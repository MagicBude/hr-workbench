...
- 每个模块保持单一职责；文件头只说明稳定职责与关键边界，不记录已失效的开发阶段。
- 禁止使用 `eval`、`with` 等危险语法。

### 1.2 注释
- 注释用于解释业务口径、边界条件和取舍，不逐行翻译代码。
- 命名已经清楚的函数无需仪式性注释；迁移、金额口径、日期边界等高风险逻辑必须说明原因。
- 删除随版本失效的 “Phase N 新增”“临时修复”等过程性描述；历史过程写入提交记录和计划文档。
- 注释应与实现同一提交更新，避免保留听起来正确但已经不成立的承诺。

### 1.3 命名
- 变量/函数：小驼峰 `restSeedMinutes`、`buildPayrollRecord`。
- 常量：大写下划线 `STORAGE_PREFIX`、`BIG_SICKNESS`。
- 文件：小写下划线 `attendance.js`、`store.js`。
- id：带前缀 + 时间戳，`e_` 员工、`a_` 考勤、`p_` 薪资、`org_` 组织。

### 1.4 样式
- 主题级颜色使用 `:root` CSS 变量（`--blue`、`--ok`、`--danger` 等）；业务状态色集中在 `config.js`，局部装饰色应复用现有色阶。
...
- 开启“调休余额严格校验”时，实际调休时长不得超过员工当前可用余额。
- 上午/下午空格单击快捷记为出勤，已有状态单击打开状态选择条，Shift + 单击保留依次循环；加班行单击直接在“加 / 空”间切换，框选多个格子使用状态工具条批量应用。
- 选择“调”时，如果余额不足默认半天但仍大于 0，系统打开时长编辑器，并以当前可用余额作为建议时长；余额为 0 时不写入调休，用户可直接从状态条选择其他状态。
- 关闭严格校验后，仍按默认时长写入并允许余额为负数，同时给出透支提示。
...
每个可独立验证的改动形成一个内聚提交；不要把无关功能、格式化和重构混在同一提交中。
