  ...
  state.data.employees
    ...
    .forEach((e, i) => {
      rows.push([
        ...
        e.leaveDate || "",
        e.baseSalary,
        round1(computeRestMinutes(e.id) / 60),
        e.insuranceBase ?? "",
      ]);
    });
...
export function exportAttendanceXlsx(month) {
  ...
  state.data.employees
    .filter((e) => !e.deletedAt && isEmployeeActiveInMonth(e, month))
    .forEach((e, i) => {
      ...
      SHIFTS3.forEach(([sh, label], si) => {
        ...
        for (let d = 1; d <= N; d++) {
          ...
          const value =
            cell && typeof cell === "object" ? cell[sh] || "" : sh === "am" ? cell || "" : "";
          row.push(
            value && typeof value === "object"
              ? `${value.s || ""}${value.min != null ? `(${value.min}分钟)` : ""}`
              : value,
          );
        }
