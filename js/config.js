/*
 * config.js — 全局常量和新组织默认规则
 *
 * 输入：无运行时输入。
 * 输出：考勤状态、颜色、节假日、默认组织设置和演示薪资参数。
 * 协作：业务模块读取这些常量，store.js 用默认值初始化和迁移旧数据。
 *
 * 修改注意：状态、单位和默认设置会影响存储结构、领域计算、导入校验与导出字段，
 * 不能把这里的改动当作单纯换文案，必须同步测试和文档。
 */
...
// 考勤的 9 种状态。具体可选范围由日期类型、时段和组织开关共同决定。
